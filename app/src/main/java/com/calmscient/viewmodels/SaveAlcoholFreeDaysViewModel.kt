...
@HiltViewModel
class SaveAlcoholFreeDaysViewModel @Inject constructor(private val repository: TakingControlRepository) : ViewModel() {

    val saveResponseLiveData: MutableLiveData<SaveAlcoholFreeDayResponse?> = MutableLiveData()
    ...
    val failureResponseData: MutableLiveData<SaveAlcoholFreeDayResponse?> = MutableLiveData()


    private var lastClientId: Int = -1
    private var lastPatientId: Int = -1
    private var lastPlId: Int = -1
    private var lastPvcFlag: String = ""
    private var lastGoalSetupId: Int = -1
    private var lastAccessToken: String = ""
    private var lastDays: List<String> = emptyList()
    private var lastMonths: List<String> = emptyList()

    // Function to save AlcoholFreeDays
    fun saveAlcoholFreeDays(clientId: Int,days: List<String>,goalSetupId: Int,months: List<String>,patientId: Int, plId: Int,pvcFlag: String,accessToken:String) {
        ...
        viewModelScope.launch {
            try {
                val request = SaveAlcoholFreeDayRequest(clientId,days,goalSetupId,months,patientId,plId,pvcFlag)
                val response = repository.savePatientAlcoholFreeDays(request,accessToken)
    ...
    private suspend fun handleResponse(call: Call<SaveAlcoholFreeDayResponse>) {
    ...
    fun retrySaveAlcoholFreeDays() {
        if(lastPatientId>0 && lastPlId>0 && lastClientId >0 && lastAccessToken.isNotEmpty())
        {
            saveAlcoholFreeDays(lastClientId,lastDays,lastGoalSetupId,lastMonths,lastPatientId,lastPlId,lastPvcFlag,lastAccessToken)
        }
    }
