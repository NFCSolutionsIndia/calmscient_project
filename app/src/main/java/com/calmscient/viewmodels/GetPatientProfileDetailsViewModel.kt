...
@HiltViewModel
class GetPatientProfileDetailsViewModel @Inject constructor(private val repository: UserProfileRepository) : ViewModel() {

    val saveResponseLiveData: MutableLiveData<GetPatientProfileDetailsResponse?> = MutableLiveData()
    ...
    val failureResponseData: MutableLiveData<GetPatientProfileDetailsResponse?> = MutableLiveData()
    ...
    fun getPatientProfileDetails(patientId: Int,accessToken:String) {
        ...
        viewModelScope.launch {
            try {
                val request = GetPatientProfileDetailsRequest(patientId)
                val response = repository.getPatientProfileDetails(accessToken,request)
                ...
                Log.e("GetPatientProfileDetailsViewModel", "Timeout Exception: ${e.message}")
            } catch (e: Exception) {
                e.printStackTrace()
    ...
    private suspend fun handleResponse(call: Call<GetPatientProfileDetailsResponse>) {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    if (isSuccess) {
                        ...
                        Log.d("GetPatientProfileDetailsViewModel ", "Success: ${response.body()}")
    ...
    fun retryGetPatientProfileDetails() {
        if( lastPatientId>0 && lastAccessToken.isNotEmpty())
        {
            getPatientProfileDetails(lastPatientId,lastAccessToken)
        }
    }
