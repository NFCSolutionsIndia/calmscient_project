...
@HiltViewModel
class SaveDrinkCountGoalSetupViewModel @Inject constructor(private val repository: TakingControlRepository) : ViewModel() {

    val saveResponseLiveData: MutableLiveData<SaveGoalSetupResponse?> = MutableLiveData()
    ...
    val failureResponseData: MutableLiveData<SaveGoalSetupResponse?> = MutableLiveData()


    private var lastAccessToken: String = ""
    private lateinit var lastRequest: SaveGoalSetupRequest

    // Function to  saveMyDrinkHabitAnswer
    fun saveDrinkCountGoalSetup(request: SaveGoalSetupRequest,accessToken:String) {
        ...
        viewModelScope.launch {
            try {
                val response = repository.saveDrinkCountGoalSetup(request,accessToken)
                Log.d("SaveDrinkCountGoalSetupViewModel", "Response: $response")
                handleResponse(response)
            } catch (e: SocketTimeoutException) {
                // Handle timeout exception
                failureLiveData.postValue("Timeout Exception: ${e.message}")
                Log.e("SaveDrinkCountGoalSetupViewModel", "Timeout Exception: ${e.message}")
            } catch (e: Exception) {
                e.printStackTrace()
    ...
    private suspend fun handleResponse(call: Call<SaveGoalSetupResponse>) {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    if (isSuccess) {
                        ...
                        Log.d("SaveDrinkCountGoalSetupViewModel ", "Success: ${response.body()}")
    ...
    fun retrySaveDrinkCountGoalSetup() {
        if( lastAccessToken.isNotEmpty())
        {
            saveDrinkCountGoalSetup(lastRequest,lastAccessToken)
        }
    }
