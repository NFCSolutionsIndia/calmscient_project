...
@HiltViewModel
class GetEventTrackerViewModel @Inject constructor(private val repository: TakingControlRepository) : ViewModel() {

    val saveResponseLiveData: MutableLiveData<GetEventsListResponse?> = MutableLiveData()
    ...
    val failureResponseData: MutableLiveData<GetEventsListResponse?> = MutableLiveData()
    ...
    fun getEventTackerData(  activityDate: String, clientId:Int, patientId: Int,plId: Int, accessToken:String) {
        ...
        viewModelScope.launch {
            try {
                val request = GetEventsListRequest(activityDate,clientId,patientId,plId)
                val response = repository.getEventTackerData(request,accessToken)
                Log.d("GetEventTrackerViewModel", "Response: $response")
                handleResponse(response)
            } catch (e: SocketTimeoutException) {
                // Handle timeout exception
                failureLiveData.postValue("Timeout Exception: ${e.message}")
                Log.e("GetEventTrackerViewModel", "Timeout Exception: ${e.message}")
            } catch (e: Exception) {
                e.printStackTrace()
    ...
    private suspend fun handleResponse(call: Call<GetEventsListResponse>) {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    if (isSuccess) {
                        ...
                        Log.d("GetEventTrackerViewModel ", "Success: ${response.body()}")
    ...
    fun retryGetDrinkTackerData() {
        if(lastPlId>0 && lastPatientId>0 && lastClientId>0 && lastActivityDate.isNotEmpty() && lastAccessToken.isNotEmpty())
        {
            getEventTackerData(lastActivityDate,lastClientId,lastPatientId,lastPlId,lastAccessToken)
        }
    }
