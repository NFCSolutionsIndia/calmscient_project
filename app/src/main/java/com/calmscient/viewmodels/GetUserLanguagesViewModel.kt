...
@HiltViewModel
class GetUserLanguagesViewModel @Inject constructor(private val repository: UserProfileRepository) : ViewModel() {

    val saveResponseLiveData: MutableLiveData<GetUserLanguagesResponse?> = MutableLiveData()
    ...
    val failureResponseData: MutableLiveData<GetUserLanguagesResponse?> = MutableLiveData()
    ...
    fun getUserLanguages(clientId:Int, patientId: Int,accessToken:String) {
        ...
        viewModelScope.launch {
            try {
                ...
                val response = repository.getUserLanguages(request,accessToken)
                Log.d("GetUserLanguagesViewModel", "Response: $response")
                handleResponse(response)
            } catch (e: SocketTimeoutException) {
                // Handle timeout exception
                failureLiveData.postValue("Timeout Exception: ${e.message}")
                Log.e("GetUserLanguagesViewModel", "Timeout Exception: ${e.message}")
            } catch (e: Exception) {
                e.printStackTrace()
    ...
    private suspend fun handleResponse(call: Call<GetUserLanguagesResponse>) {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    if (isSuccess) {
                        ...
                        Log.d("GetUserLanguagesViewModel ", "Success: ${response.body()}")
    ...
    fun retryGetUserLanguages() {
        if( lastPatientId>0 && lastClientId>0  && lastAccessToken.isNotEmpty())
        {
            getUserLanguages(lastClientId,lastPatientId,lastAccessToken)
        }
    }
